...
사용 가능한 label 예시(짧은 대문자 권장):
- PER: 인물 이름
- LOC: 장소·지명
- ORG: 조직·회사·기관
- DAT: 날짜·기간(예: 2026-02-01, 지난달)
- TIM: 시간·시각(예: 오후 2시, 10:30)
- MON: 금액·화폐(예: 3,000,000원)
- QTY: 수량·횟수·개수(예: 10번, 5개)
- PCT: 비율·퍼센트(예: 18%, 50%)
- FOOD: 음식·식재료·메뉴(예: 치킨, 커피)
- PRODUCT: 제품·서비스·앱·버전(예: PROTOCL 앱, 코그로보, API)
- EVENT: 행사·이벤트·프로모션(예: 미팅, 세일)
- TECH: 기술·기능·도구·프레임워크(예: SQL, RAG, NER)
- ID: 식별자·계정·주문번호·유저ID(예: user_id, 주문번호)
- URL: URL·웹주소
- EMAIL: 이메일 주소
- PHONE: 전화번호·연락처
- MISC: 그 외 고유 정보(분류가 애매하거나 위 예시로 커버하기 어려운 항목)
...
- label은 위 예시 목록의 태그들(PER/LOC/ORG/DAT/TIM/MON/QTY/PCT/FOOD/PRODUCT/EVENT/TECH/ID/URL/EMAIL/PHONE) 중에서 우선 선택하고,
  필요하다면 의미에 맞는 유사 태그/세분 라벨을 추가해도 됩니다. 분류가 애매하면 MISC로 처리하세요.
