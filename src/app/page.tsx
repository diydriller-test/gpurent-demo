...
const FAQ_ITEMS: { q: string; a: string }[] = [
  ...
  {
    q: "사용자가 입력한 프롬프트나 민감정보, 이미지들이 서버에 저장되나요?",
    a: "아닙니다. AI API 오마카세는 로그인 사용자 정보 외에 사용자가 입력하신 프롬프트, 개인정보, 민감정보, 이미지, 업로드 파일 등 어떠한 사용자 데이터도 서버에 저장하지 않습니다. 제공 중인 9종 API 모두 별도의 저장(storage) 기능을 두지 않아 요청 처리 후 모든 입력 데이터는 서버에 남지 않으며, 업로드하신 파일은 처리에 필요한 순간에만 임시(temp) 영역에 머문 뒤 호출 종료와 함께 자동으로 폐기됩니다. 따라서 사용자께서는 별도의 삭제 요청 절차 없이도 입력하신 모든 데이터가 안전하게 처리된다고 신뢰하실 수 있습니다.",
  },
  {
    q: "월 정액인가요?",
